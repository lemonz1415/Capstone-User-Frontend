...
export default function withAuth(Component: React.ComponentType, redirectIfLoggedIn?: string) {
  return function AuthenticatedComponent(props: any) {
    ...
    useEffect(() => {
      if (!isLoading) {
        if (isLoggedIn && ["/auth/login", "/auth/register", "/auth/verify-email"].includes(pathname) && 
        pathname !== "/exam" && 
      ...
      sessionStorage.removeItem("justLoggedIn");
        ...
        if (!isLoggedIn && !isOpen && !["/auth/login", "/auth/register", "/auth/verify-email"].includes(pathname)) {
    ...
    if (isLoading || (!isLoggedIn && !["/auth/login", "/auth/register", "/auth/verify-email"].includes(pathname))) {
